...
class MusicLEDController:
    ...
    def sync_and_play(self, audio_source):
        ...
        # FIX #7: drift máximo tolerado antes de pular envio do frame
        max_drift_s    = self.config.PLAYBACK_MAX_DRIFT_FRAMES * frame_duration
        frames_skipped = 0
        ...
        try:
            ...
            for i, (mode, led_values) in enumerate(led_patterns):
                ...
                drift         = time.time() - expected_time
                # FIX #7: se lag acumulou mais que o limite, pula envio mas mantém tempo
                ...
                if i % self.fps == 0:
                    ...
                    skip_s  = _c(f"  skip:{frames_skipped}", C.BYELLOW) if frames_skipped else ""
                    print(
                        f"  {bar}  "
