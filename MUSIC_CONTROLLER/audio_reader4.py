...
class EffectConfig:
    ...
    SEG_MIN_ENERGY_THRESHOLD: float = 0.08
...
class MusicLEDController:
    ...
    def _snap_to_beats(
        ...
        for t in transitions:
            original_dur = t.frame_end - t.frame_start
            snapped_start = nearest_beat(t.frame_start)

            # Quantiza duração ao múltiplo de beats mais próximo da dur. desejada
            n_beats = max(1, round(original_dur / beat_period))
            snapped_end = beat_after(snapped_start, n_beats)

            # Garante limites de duração configurados
            dur = snapped_end - snapped_start
            if dur < min_frames:
                snapped_end = snapped_start + min_frames
            elif dur > max_frames:
                snapped_end = snapped_start + max_frames

            snapped_end = min(snapped_end, n_frames - 1)

            snapped.append(TransitionEvent(
                frame_start     = snapped_start,
                frame_end       = snapped_end,
        ...
        # Remove sobreposições: se uma transição começa antes da anterior terminar,
        # recua o início para o beat logo após o fim da anterior.
        cleaned = [snapped[0]]
        for t in snapped[1:]:
            ...
            if t.frame_start < prev_end:
                ...
                dur = t.frame_end - t.frame_start
                new_end = min(new_start + dur, n_frames - 1)
    ...
    def sync_and_play(self, audio_source):
        ...
        print(_c("    Pressione Ctrl+C para parar\n", C.DIM))
        ...
        def _pwm_viz(values: np.ndarray) -> str:
            parts = []
            for idx, v in enumerate(values):
                level  = int(np.clip(v, 0, 255))
                block  = _BLOCKS[min(int(level / 255 * 8), 8)]
                is_edge = (idx == 0 or idx == len(values) - 1)
                if level == 0:
                    color = C.DIM
                elif level > 200:
                    color = C.BRED if is_edge else C.BMAGENTA
                elif level > 100:
                    color = C.BYELLOW if is_edge else C.BCYAN
                else:
                    color = C.BBLUE
                parts.append(_c(f"{block}{block}", color))
            return " ".join(parts)
        ...
        try:
            ...
            for i, (mode, led_values) in enumerate(led_patterns):
                ...
                if i % self.fps == 0:
                    ...
                    bar     = _c("█" * filled, C.BMAGENTA) + _c("░" * (20 - filled), C.DIM)
                    viz     = _pwm_viz(led_values)
                    print(
                        f"  {bar}  "
                        f"{_c(f'{elapsed:5.1f}s', C.BYELLOW)} / {_c(f'{duration:.1f}s', C.DIM)}"
                        f"  [{viz}]",
                        end="\r",
                    )
