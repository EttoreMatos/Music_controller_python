...
class MusicLEDController:
    ...
    def process_audio_file(self, audio_file):
        ...
        log_sub(f"BPM detectado: {_c(f'{tempo_bpm:.1f}', C.BMAGENTA, C.BOLD)}")
        ...
        log_step("Telemetria de saturação")
        sat_str  = "  ".join(
            _c(f"L{i+1}:{sat[i]:.0f}%",  C.BRED    if sat[i]  > 30 else C.BGREEN)
            for i in range(num_leds)
        )
        dark_str = "  ".join(
            _c(f"L{i+1}:{dark[i]:.0f}%", C.BYELLOW if dark[i] > 60 else C.BGREEN)
            for i in range(num_leds)
        )
        log_sub(f"Saturado  ≥245: {sat_str}")
        log_sub(f"Escuro    ≤5:   {dark_str}")
    ...
    def sync_and_play(self, audio_source):
        ...
        def _pwm_viz(values: np.ndarray) -> str:
            return " ".join(
                _c("▌", C.BMAGENTA if v > 180 else C.BCYAN if v > 60 else C.DIM)
                for v in values
            )
